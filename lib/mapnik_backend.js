    ...
    if (!this._pool) this._pool = Pool({
        create: function(callback) {
            ...
            map.fromString(xml, opts, function(err, map) {
                if (err) return callback(err);
                // https://github.com/mapbox/tilelive-mapnik/issues/20
                callback(null, map);
            });
        },
...
MapnikSource.prototype._createMetatileCache = function() {
    ...
    this._tileCache = new LockingCache(function(key) {
        ...
        var keys = source._renderMetatile({
            // Force metatile 1 for data tiles.
            metatile: coords[0] === 'data' ? 1 : source._uri.query.metatile,
            // @TODO, does tileSize mean anything to a data tile?
            tileSize: coords[0] === 'data' ? 256 : source._uri.query.tileSize,
            format: coords[0],
            z: +coords[1],
            x: +coords[2],
            y: +coords[3]
...
MapnikSource.prototype.getData = function(z, x, y, callback) {
    z = +z; x = +x; y = +y;
    if (isNaN(z) || isNaN(x) || isNaN(y)) {
        return callback(new Error('Invalid coordinates: '+z+'/'+x+'/'+y));
    }

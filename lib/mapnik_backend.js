...
MapnikSource.prototype._createMetatileCache = function() {
    ...
    this._tileCache = new LockingCache(function(key) {
        ...
        var keys = source._renderMetatile({
            ...
            metatile: coords[0].indexOf('data') === 0 ? 1 : source._uri.query.metatile,
            // tileSize does not mean anything to a data tile.
            tileSize: coords[0].indexOf('data') === 0 ? 256 : source._uri.query.tileSize,
            format: coords[0],
            z: +coords[1],
            x: +coords[2],
            y: +coords[3]

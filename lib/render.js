...
function getImage(image, options, x, y, callback) {
    // Data tiles use a different API than images/grids.
    if (options.format === 'data') try {
        var solid = image.isSolid() && JSON.stringify(_(image.toDatasources()).reduce(function(memo, ds, name) {
            var features = ds.features();
            if (features.length) memo[name] = [_(features).pluck('__id__')];
            return memo;
        }, {}));
        if (solid === '{}') solid = '0,0,0,0'; // Fake empty RGBA for now.
    ...
    // Data tiles (json debug).
    if (options.format === 'data.json') try {
        var datajson = _(image.toDatasources()).reduce(function(memo, ds, name) {
            memo[name] = ds.features().length;
            return memo;
        }, {});
        return callback(null, datajson);
    } catch(err) { return callback(err); }
    // Data tile proxy => png.
    if (options.format === 'data.png') options.format = 'png';
...
MapnikSource.prototype._renderMetatile = function(options, callback) {
    ...
    if (options.format === 'utf') {
        ...
        options.tolerance = 5;
        options.headers = { 'Content-Type': 'application/x-protobuf', 'Content-Encoding': 'deflate' };
        var image = new mapnik.DataTile(options.z, options.x, options.y);
    } else if (options.format === 'data.json') {
        options.tolerance = 5;
        options.headers = { 'Content-Type': 'text/javascript; charset=utf-8' };
        var image = new mapnik.DataTile(options.z, options.x, options.y);
    } else if (options.format === 'data.png') {
        // higher value more coordinates will be skipped
        options.tolerance = 5;

        // lower the value means less precisions and more
        // coordinates will be skipped
        // defaults to the value of options.z
        //options.path_multiplier = 16;

        // make larger than zero to enable
        options.simplify = 0.0;
        // zhao-saalfeld is default
        // other options are 'radial-distance' and 'visvalingam-whyatt'
        options.simplify_algorithm = 'radial-distance'
        ...
        if (options.format.indexOf('png') != -1) {
            ...
            options.headers = { 'Content-Type': mime.lookup(options.format) };
        }
    ...
    process.nextTick(function() {
        ...
        try {
            source._pool.acquire(function(err, map) {
                ...
                // "Proxy" data tile render.
                if (options.format === 'data.png') return map.render(image, options, function(err, tile) {
                    if (err) {
                        process.nextTick(function() {
                    ...
                    //console.log('data: '+options.z+'/'+options.x+'/'+options.y+': ' + tile.getData().length + " bytes issolid: "+tile.isSolid());
                    tile.render(map, new mapnik.Image(meta.width, meta.height), function(err, image) {
                        process.nextTick(function() {
